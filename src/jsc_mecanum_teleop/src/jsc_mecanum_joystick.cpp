...
using namespace jsc::mecanum::teleop;

#define JSC_MECANUM_JOYSTICK_UNSCALED_MAX    32767.0
...
void MecanumbotJoystick::update()
{
    if (device_handle_ < 0) {
        ...
        if (deadzone_ > 1.0) {
            deadzone_ /= JSC_MECANUM_JOYSTICK_UNSCALED_MAX;
        }
        if (deadzone_ < 0.0) {
            deadzone_ = 0.0;
        }
        scale_ = -1.0f / (1.0f - deadzone_) / JSC_MECANUM_JOYSTICK_UNSCALED_MAX;
        unscaled_deadzone_ = JSC_MECANUM_JOYSTICK_UNSCALED_MAX * deadzone_;
    }
